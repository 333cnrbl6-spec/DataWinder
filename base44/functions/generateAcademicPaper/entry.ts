...
Return a valid JSON object with these exact keys (all values are long strings of continuous prose):
{
  ...
  "word_count_estimate": 9000
    ...
    // Generate single AI image for figure (skip extra images to avoid timeout)
    ...
    try {
      ...
      if (imgResult?.url) {
        generatedFigures.push({
          id: 'range_map',
          type: 'image',
          title: 'Geographic Range Distribution',
          description: 'Species range map from IUCN and field observations',
          image_url: imgResult.url,
          generated_at: new Date().toISOString()
        });
        console.log('Generated figure: range_map');
      }
    } catch (imgErr) {
      console.warn('Failed to generate range map figure:', imgErr.message);
    }
